...
with lib;
with builtins; let
  cfg = config.vim.code.languages.nix;
...
in {
  options.vim.code.languages.nix = {
    ...
    format = {
      enable = mkOption {
        ...
        type = types.bool;
        default = config.vim.code.languages.enableFormat;
      };
      type = mkOption {
  ...
  config = mkIf cfg.enable (mkMerge [
    ...
    (mkIf cfg.lsp.enable {
      ...
      vim.code.lsp.lspconfig.sources.nix-lsp = servers.${cfg.lsp.server}.lspConfig;
    })

    (mkIf (cfg.format.enable && !servers.${cfg.lsp.server}.internalFormatter) {
      vim.code.lsp.null-ls.enable = true;
      vim.code.lsp.null-ls.sources.nix-format = formats.${cfg.format.type}.nullConfig;
    })
  ]);
}
